...
from arbfinder import arb_finder, config as config_module, watch as watch_module, utils
...
For more help on a subcommand: arbfinder <subcommand> --help
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"ArbFinder Suite v{__version__}"
    )
    ...
    search_parser.add_argument("--live-limit", type=int, default=80, help="Max live listings per provider")
    search_parser.add_argument("--comp-limit", type=int, default=150, help="Max sold comps to fetch")
    search_parser.add_argument("--sim-threshold", type=int, default=86, help="Similarity threshold 0-100")
    search_parser.add_argument("--threshold-pct", type=float, default=20.0, help="Min discount percentage")
    ...
    search_parser.add_argument("-i", "--interactive", action="store_true", help="Interactive TUI mode")
    search_parser.add_argument("-q", "--quiet", action="store_true", help="Suppress progress output")
    ...
    watch_parser.add_argument("--interval", type=int, default=3600, help="Check interval in seconds")
    watch_parser.add_argument("--threshold-pct", type=float, default=20.0, help="Min discount percentage")
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage ArbFinder configuration"
    )
    ...
    db_parser = subparsers.add_parser(
        ...
        description="Manage ArbFinder database"
    )
    ...
    db_clean_parser.add_argument("--days", type=int, default=30, help="Remove entries older than N days")
...
def handle_server(args: argparse.Namespace) -> int:
    ...
    try:
        import uvicorn
        from arbfinder.api import main as api_main
        
        print(f"Starting server at http://{args.host}:{args.port}")
        uvicorn.run(
            "arbfinder.api.main:app",
            host=args.host,
            port=args.port,
            reload=args.reload
        )
