...
async def recommend_model(
    ...
    if max_cost == 0.0:
        ...
        candidates = [
            ...
            if float(m.pricing.get("prompt", "999")) <= max_cost
        ]
...
async def compare_models(
    ...
    if comparison["models"]:
        comparison["metrics"]["avg_context_length"] = (
            total_context / len(comparison["models"])
        )
...
async def get_free_models_cached(ttl: int = 86400) -> List[ModelInfo]:
    ...
    if (_free_models_cache is None or
        current_time - _cache_timestamp > ttl):
