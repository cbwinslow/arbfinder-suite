...
def db_upsert_listing(db_path: str, listing: Listing) -> None:
    with sqlite_conn(db_path) as conn:
        ...
        c.execute(
            ...
            """,
            (
                listing.source, listing.url, listing.title, listing.price,
                listing.currency, listing.condition, listing.timestamp,
...
class PoliteClient:
    def __init__(self, ua: str = DEFAULT_UA, timeout: float = HTTP_TIMEOUT):
        self._client = httpx.AsyncClient(headers={"User-Agent": ua}, timeout=timeout, follow_redirects=True)
        ...
        async with self._sem:
            ...
            for attempt in range(1, RETRY_MAX + 1):
                try:
                    ...
                    logger.warning("GET %s failed (%s). retry %d in %.1fs", url, e, attempt, backoff)
...
class EbaySoldComps(Provider):
    ...
    async def search(self, query: str, limit: int = 120):
        ...
        for block in re.split(r"<li class=\\\"s-item[\\s\\S]*?>", html)[1:]:
            ...
            items.append(Listing(self.name, url_item, title_raw, price, currency, "sold", meta={"q": query}))
            if len(items) >= limit:
                break
        return items
        ...
        while len(items) < limit and page <= 3:
            ...
            for card in html.split("product-card"):
                ...
                try:
                    ...
                    continue
                items.append(Listing(self.name, url_item, title, price, "USD", "live", meta={"q": query, "page": page}))
                if len(items) >= limit:
                    break
            page += 1
        return items
    ...
    async def search(self, query: str, limit: int = 60):
        ...
        while len(items) < limit and page <= 2:
            ...
            for block in re.split(r"<div class=\\\"auction-card[\\s\\S]*?>", html)[1:]:
                ...
                m_url = re.search(r"href=\\\"(/index\\\\.cfm\\?fa=Main\\\\.Item&itemid[^\"]+)\\\"", block)
        ...
        if p.suffix.lower() == ".csv":
            ...
            with open(p, newline="", encoding="utf-8") as f:
                for row in csv.DictReader(f):
                    try:
                        items.append(Listing(self.name, row.get("url",""), row.get("title",""), float(row.get("price",0)), row.get("currency","USD"), row.get("condition","live")))
                    except Exception:
                        pass
            ...
            for obj in data:
                try:
                    items.append(Listing(self.name, obj.get("url",""), obj.get("title",""), float(obj.get("price",0)), obj.get("currency","USD"), obj.get("condition","live")))
                except Exception:
                    pass
        return items[:limit]
...
def match_comps_to_live(live: List[Listing], comps: Dict[str, Comp], sim_threshold: int = 86) -> List[Dict[str, Any]]:
