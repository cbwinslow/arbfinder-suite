...
export default function AlertsPage() {
  ...
  const toggleAlertStatus = async (alertId: number, currentStatus: string) => {
    ...
    try {
      const response = await fetch(`${apiBase}/api/alerts/${alertId}/${action}`, {
        method: 'PATCH',
      });
    ...
    <main className="mx-auto max-w-7xl p-6 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-2">Get notified when items match your criteria</p>
        </div>
      ...
      <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4">
        <h3 className="text-blue-400 font-semibold mb-2">📢 How Price Alerts Work</h3>
        <p className="text-gray-300 text-sm">
          Set up alerts to be notified when new listings match your search criteria and price range. 
          You can receive alerts via email, webhook, or social media. The system checks for new matches 
          automatically and sends you a notification when items are found. Perfect for finding deals 
          on specific items within your budget!
        </p>
      </div>
      ...
      {showForm && (
        <div className="bg-neutral-900 rounded-lg p-6 border border-orange-500/20">
          <h2 className="text-xl font-semibold mb-4 text-orange-400">Create New Alert</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="text-sm text-gray-400 block mb-2">Search Query *</label>
              <input
            ...
            <div>
              <label className="text-sm text-gray-400 block mb-2">Minimum Price ($)</label>
              <input
                className="w-full bg-neutral-800 p-3 rounded border border-gray-700 focus:border-orange-500 focus:outline-none transition"
                placeholder="200.00"
                type="number"
            ...
            <div>
              <label className="text-sm text-gray-400 block mb-2">Maximum Price ($)</label>
              <input
                className="w-full bg-neutral-800 p-3 rounded border border-gray-700 focus:border-orange-500 focus:outline-none transition"
                placeholder="400.00"
                type="number"
            ...
            <div>
              <label className="text-sm text-gray-400 block mb-2">
                {notificationMethod === 'email' ? 'Email Address' :
                 notificationMethod === 'webhook' ? 'Webhook URL' :
                 notificationMethod === 'twitter' ? 'Twitter Handle' :
                 'Facebook Page'} *
              </label>
              <input
                className="w-full bg-neutral-800 p-3 rounded border border-gray-700 focus:border-orange-500 focus:outline-none transition"
                placeholder={
                  notificationMethod === 'email' ? 'you@example.com' :
                  notificationMethod === 'webhook' ? 'https://your-webhook.com/alert' :
                  notificationMethod === 'twitter' ? '@yourusername' :
                  'YourPageName'
                }
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {alerts.filter(a => a.status !== 'deleted').map((alert) => (
            <div
              key={alert.id}
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className={`text-xs font-semibold px-2 py-1 rounded border ${getStatusColor(alert.status)}`}>
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-green-400 font-semibold">
                        {alert.min_price !== null ? `$${alert.min_price.toFixed(2)}` : 'Any'} - 
                        {alert.max_price !== null ? ` $${alert.max_price.toFixed(2)}` : ' Any'}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-blue-400">
                        {getNotificationIcon(alert.notification_method)} {alert.notification_method}
                      </span>
                    </div>
                    
                    <div className="flex items-center gap-2">
                      <span className="text-gray-400">Target:</span>
                      <span className="text-gray-300">{alert.notification_target}</span>
                    </div>
                    ...
                    {alert.last_triggered_at && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-yellow-400">
                          {new Date(alert.last_triggered_at * 1000).toLocaleString()}
                        </span>
                      </div>
                    )}
                  </div>
        ...
        {!loading && alerts.filter(a => a.status !== 'deleted').length === 0 && (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm mt-2">Click "Create Alert" to get started</p>
          </div>
        )}
      </div>
    </main>
