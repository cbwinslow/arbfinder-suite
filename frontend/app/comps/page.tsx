...
export default function CompsPage() {
  ...
  const searchComps = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`${apiBase}/api/comps/search?q=${encodeURIComponent(searchQuery)}`)
      const data = await response.json()
      setComps(data)
    } catch (error) {
    ...
    <main className="mx-auto max-w-7xl p-6 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-2">Historical price data from eBay sold listings</p>
        </div>
      ...
      <div className="bg-neutral-900 rounded-lg p-6 border border-blue-500/20">
        <h2 className="text-xl font-semibold mb-4 text-blue-400">🔍 Search Comparables</h2>
      ...
      <div>
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="mt-3 text-xs text-gray-500">
                    Last updated: {new Date(comp.timestamp * 1000).toLocaleString()}
                  </div>
                </div>
              </div>
            </div>
        ...
        {!loading && comps.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm mt-2">Run the crawler to collect price data</p>
          </div>
        )}
      </div>
    </main>
