    ...
    <main className="mx-auto max-w-7xl p-6 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-2">Schedule bids to win auctions at the last moment</p>
        </div>
      ...
      <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4">
        <h3 className="text-blue-400 font-semibold mb-2">🎓 How Sniping Works</h3>
        <p className="text-gray-300 text-sm">
          Auction sniping is the practice of placing a bid at the last possible moment before an 
          auction closes. This prevents others from having time to counter-bid. Enter the auction 
          details, your maximum bid, and how many seconds before the auction ends you want to place 
          the bid (lead time). The system will automatically execute the bid at the right moment.
        </p>
      </div>
      ...
      {showForm && (
        <div className="bg-neutral-900 rounded-lg p-6 border border-purple-500/20">
          <h2 className="text-xl font-semibold mb-4 text-purple-400">Schedule New Snipe</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="text-sm text-gray-400 block mb-2">Auction URL *</label>
              <input
            ...
            <div className="md:col-span-2">
              <label className="text-sm text-gray-400 block mb-2">Item Title (optional)</label>
              <input
                className="w-full bg-neutral-800 p-3 rounded border border-gray-700 focus:border-purple-500 focus:outline-none transition"
            ...
            <div>
              <label className="text-sm text-gray-400 block mb-2">Maximum Bid ($) *</label>
              <input
                className="w-full bg-neutral-800 p-3 rounded border border-gray-700 focus:border-purple-500 focus:outline-none transition"
                placeholder="150.00"
                type="number"
            ...
            <div>
              <label className="text-sm text-gray-400 block mb-2">Auction End Time *</label>
              <input
                className="w-full bg-neutral-800 p-3 rounded border border-gray-700 focus:border-purple-500 focus:outline-none transition"
                type="datetime-local"
            ...
            <div className="md:col-span-2">
              <label className="text-sm text-gray-400 block mb-2">Lead Time (seconds before end)</label>
              <input
                className="w-full bg-neutral-800 p-3 rounded border border-gray-700 focus:border-purple-500 focus:outline-none transition"
                type="number"
                min="1"
              ...
              <p className="text-xs text-gray-500 mt-1">
                Recommended: 5-15 seconds. Too early and others can outbid you; too late and the bid might not process.
              </p>
            </div>
      ...
      <div>
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className={`text-xs font-semibold px-2 py-1 rounded border ${getStatusColor(snipe.status)}`}>
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-2xl font-bold text-green-400">${snipe.max_bid.toFixed(2)}</span>
                    </div>
                    
                    <div className="flex items-center gap-2">
                      <span className="text-gray-400">Auction Ends:</span>
                      <span className="text-yellow-400">
                        {new Date(snipe.auction_end_time * 1000).toLocaleString()}
                      </span>
                    </div>
                    ...
                    {snipe.result && (
                      <div className="mt-2 p-2 bg-neutral-800 rounded text-xs">
                        <span className="text-gray-400">Result:</span> {snipe.result}
                      </div>
                    )}
        ...
        {!loading && snipes.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm mt-2">Click "Schedule Snipe" to get started</p>
          </div>
        )}
      </div>
    </main>
